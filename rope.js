class Rope
  {
    ...
    {
      //these are number of links
      this.nlink = nlink
  const group = Body.nextGroup(true);
  //composits.stack()function creates multipple rectangular bodies and store it in rects var
  ...
  this.pointA = pointA;
  //composits.chain()function creates a chain from the above rectangles
  this.body = Composites.chain(rects, 0.1, 0, -0.6, 0, {stiffness: 0.1, length: 0.1, render: {type: 'line'}});
      
  World.add(engine.world, this.body);
  
      //using constraint.create()we add constraits to the chain which coonects all the bodies of the chain like a string of beads
